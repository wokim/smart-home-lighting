...
- **그룹.** 여러 조명을 하나로 묶은 것이다. 거실, 주방처럼 공간 단위로 나눈다
...
Note: 제어 얘기의 척추다. DALI든 Hue든 Aqara든 이름만 다르고 네 가지는 똑같다. 뒤에 나오는 비교표도 기능 나열이 아니라 이 네 가지를 어떻게 처리하는지의 비교로 보면 된다. 주소는 자동으로 부여되기도 하고 손으로 정하기도 한다. 그룹과 씬은 사람이 정해야 하고, 그게 설계 단계의 시나리오다.
