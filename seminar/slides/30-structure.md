...
<img class="diagram" src="assets/diagrams/command-path.svg" style="max-height:340px">

<ul style="font-size:.82em;margin-top:.3em">
<li>유선은 컨트롤러가 <strong>명령 한 줄</strong>을 버스에 보내고, 드라이버가 각자 저장한 값으로 움직인다. 개수가 늘어도 동시다</li>
<li>무선도 <strong>그룹 명령</strong>을 쓰면 같다. 다만 허브가 조명마다 따로 명령을 보내는 구성이면 순서대로 켜진다</li>
<li>반대 방향도 있다. 조명이 지금 몇 퍼센트인지 <strong>되돌려 보고</strong>해야 앱 화면과 실제가 어긋나지 않는다</li>
</ul>

Note: 순서대로 켜지는 걸 팝콘이라고 부른다. 명령을 하나씩 보내니 그렇다. DALI는 버스가 1200bps라 명령 하나에 20에서 40밀리초 걸리고, 50개면 1초에서 2초가 걸린다. 그래서 그룹과 씬 명령이 표준에 들어 있다. 무선도 마찬가지라 허브 안에서 그룹으로 묶어야 하고, 밖에서 개별로 부르면 팝콘이 난다. 상태 보고가 없으면 벽 스위치로 끈 것을 앱이 몰라서 화면과 실제가 어긋난다.
