...
Note: 광원이 빛의 재료라면 드라이버는 그 빛을 실제로 만들어내는 장치다. 전원 변환만 하는 게 아니라 밝기와 페이드를 실행하고, DALI라면 씬 값까지 들고 있다. 그룹 명령 하나로 여러 등이 각자 다른 밝기가 되는 게 여기서 성립한다. 씬 저장은 DALI만의 것이 아니라 Zigbee(씬 클러스터), KNX도 같다. 0-10V는 주소도 저장도 없어서 한 신호선에 물린 드라이버가 전부 같은 밝기가 된다. 그룹 명령 한 번에 여러 등이 각자 다른 밝기로 움직이는 건 드라이버가 자기 몫을 기억하고 있어서다.
...
<div class="card"><h4 style="color:#ffb454;font-size:.8em;letter-spacing:.1em;margin-bottom:.5em">CC, 점광원용</h4>다운라이트·스폿. LED 전류를 직접 다루므로 <strong>미세한 디밍에 유리</strong>. 거리에 둔감. 등기구별 전류 규격(mA)을 맞춰야 하고, <strong>드라이버의 전압·전력 한계가 등기구 수를 정한다</strong></div>
...
Note: 드라이버 레이어 안의 두 부류. CC는 전류원이라 선이 길어져도 드라이버가 전압을 올려 전류를 지키고, CV는 전압원이라 선이 길면 끝에서 전압이 떨어진다. 이게 3막에서 "드라이버 위치"를 따로 얘기하는 이유다. CV 저가품은 PWM 주파수가 낮아 카메라에 줄이 가는 플리커가 흔하다. "여러 개를 한 드라이버에 묶어 비용을 줄일 수 없냐"는 질문이 자주 나온다. 원리상 직렬로 가능하다. 다만 CC 드라이버는 대체로 정격이 작고 출력 전압 범위도 좁다. 예로 든 제품은 300mA일 때 9~33V, 최대 10W라 30V짜리 다운라이트는 하나가 끝이다. 여러 개를 묶으려면 전압 범위가 넓고 정격이 큰 상업용 드라이버가 필요한데 주거 유통에서 구하기 어렵고, 더 올라가면 DC 120V(안전 특별저압)도 넘는다. 그래서 주거는 1:1을 권장한다. 질문이 나오면: 정전력(constant power) 타입이라는 세 번째 분류도 있지만 가로등 같은 고출력용이라 주거에선 거의 안 쓴다. 출력 전류를 딥스위치나 프로그래밍으로 고르는 CC 드라이버가 흔해서 등기구별 mA를 맞추기가 예전보다 쉽다. CV라고 정전류 제어가 없는 게 아니라 그 역할이 스트립 쪽으로 넘어간 것이다.
...
- **주소.** 조명 하나를 지목하는 이름이다. DALI는 드라이버마다 번호를 받고, 무선은 기기 자체가 주소다
- **그룹.** 여러 조명을 하나로 묶은 것이다. 거실, 주방처럼 공간 단위로 나누되, 하나의 조명이 여러 묶음에 속할 수 있다
- **씬.** 조명마다 밝기를 미리 정해 저장한 것이다. 영화 모드가 이것이고, 그룹으로 불러도 조명 하나만 불러도 된다
- **매핑.** 어느 버튼과 센서가 어느 씬을 부를지 정해 두는 것이다. 이걸 안 해두면 씬이 앱 안에만 남는다
- 방식이 달라도 하는 일은 같다. 차이는 **이 네 가지를 어디에 저장하고 누가 실행하느냐**다

Note: 제어 얘기의 척추다. DALI든 Hue든 Aqara든 이름만 다르고 네 가지는 똑같다. 뒤에 나오는 비교표도 기능 나열이 아니라 이 네 가지를 어떻게 처리하는지의 비교로 보면 된다. 주소는 자동으로 부여되기도 하고 손으로 정하기도 한다. 겹치는 묶음이 실무에서 유용하다. 거실 전체, 거실 간접등만, 1층 전체가 다 필요하고 같은 조명이 세 묶음에 들어간다. Hue는 이걸 방과 존으로 나눠 부르고, DALI는 그룹 16개로 처리한다. 이름만 다르고 하는 일은 같다. 그룹과 씬은 사람이 정해야 하고, 그게 설계 단계의 시나리오다. 씬이 그룹에 매여 있느냐는 방식마다 다르다. DALI는 드라이버마다 씬 값을 갖고 있고 부를 때 개별이든 그룹이든 고르면 된다. Zigbee는 씬을 그룹과 묶어 저장한다.
...
<img class="diagram" src="assets/diagrams/command-path.svg" style="max-height:300px">

<ul style="font-size:.76em;margin-top:.2em">
<li>유선은 이너 릴레이가 <strong>명령을 직접 버스에 보낸다</strong>. 드라이버는 자기에게 온 명령인지 판단해 각자 저장한 값으로 움직인다. <strong>허브가 없어도 벽 버튼은 동작한다</strong></li>
<li>앱과 음성, 자동화는 <strong>허브가 같은 버스에</strong> 명령을 보낸다. 조명이 지금 몇 퍼센트인지 <strong>되돌려 보고</strong>해야 앱 화면과 실제가 맞는다</li>
<li>무선은 <strong>허브를 반드시 거친다</strong>. 그룹으로 한 번에 보내면 동시에, 조명마다 따로 보내면 순서대로 켜진다</li>
</ul>

Note: 순서대로 켜지는 걸 팝콘이라고 부른다. 명령을 하나씩 보내니 그렇다. DALI는 버스가 1200bps라 명령 하나에 20에서 40밀리초 걸리고, 50개면 1초에서 2초가 걸린다. 그래서 그룹과 씬 명령이 표준에 들어 있다. 무선도 마찬가지라 허브 안에서 그룹으로 묶어야 하고, 밖에서 개별로 부르면 팝콘이 난다. 상태 보고가 없으면 벽 스위치로 끈 것을 앱이 몰라서 화면과 실제가 어긋난다. 게이트웨이는 버스에 명령을 보내기도 하고 남이 보낸 명령을 같이 듣기도 한다. 그래서 벽 버튼으로 조작해 허브를 거치지 않아도 앱 상태가 따라온다. 버스가 공유선이라 공짜로 얻는 것이고, 무선은 스위치가 허브에 따로 보고하지 않으면 알 방법이 없다. 정확히 하자면 DALI-2 규격은 입력 장치가 이벤트만 보내고 애플리케이션 컨트롤러가 명령을 내리게 되어 있다. 다만 시중 커플러는 그 컨트롤러를 안에 품고 나와서 공장 기본값으로 명령을 직접 보낸다. 질문이 나오면 이렇게 답한다.
...
- 그래서 **페이드 아웃이 원리적으로 불가능하다**. 전기를 끊는 순간 조명은 그냥 꺼진다
...
<img class="diagram" src="assets/diagrams/platform-stack.svg" style="max-height:250px">

- 스위치·센서·조명이 모두 연결되는 **허브**. 씬, 자동화, 센서 로직이 여기서 돈다
- 조명만 보고 정하면 안 된다. 냉난방·커튼·센서가 **나중에 같은 자리**에 붙는다
- 조명이 두 플랫폼에 나뉘면 **하나의 씬으로 함께 다룰 수 없다**. 허브끼리 붙이는 것도 간단하지 않아서, 처음부터 **하나로 모으는 편이 낫다**
...
- 플랫폼은 보통 **Apple Home · Google Home과 양방향 연동**된다. 그러면 일반적인 조작은 별도 앱 없이 폰 기본 앱과 음성으로 된다
- 음성 제어는 Google이, 홈 앱의 완성도는 Apple이 낫다는 평이 많다
- **Matter**는 이 연동을 표준화하는 규격이다. Matter 브릿지를 쓰면 플랫폼의 기기를 Matter 기기처럼 Apple Home·Google Home에 노출할 수 있다. 다만 기기가 보이는 것과 씬·자동화로 유기적으로 엮이는 것은 별개라, 그 깊이는 여전히 플랫폼 몫이다
- Matter에도 그룹과 씬이 **규격에는** 있다. 다만 제품과 플랫폼이 아직 잘 쓰지 않아, 여러 등을 한 번에 켜면 **하나씩 순서대로** 켜지는 일이 흔하다

Note: Matter 그룹·씬 질문이 나오면. 최신 버전은 1.5.1이고 그룹과 씬은 규격에 다 들어 있다. 문제는 시장이다. 씬 기능은 얼마 전까지 인증 자체가 안 되던 항목이라 제품들이 넣을 이유가 없었고, Apple Home과 Google Home은 지금도 Matter의 씬 대신 자기네 방식으로 씬을 돌린다. 그래서 Matter 전구 여러 개를 한 번에 켜면 하나씩 순서대로 켜지는 일이 흔하다. 규격에 없어서가 아니라 시장이 아직 안 따라온 것이다. Zigbee 그룹이나 DALI 씬은 20년 넘게 굴러온 기능이라 이 부분은 아직 격차가 있다. 그래서 씬은 플랫폼에 맡기고 Matter는 폰과 음성으로 내보내는 통로로 쓰는 게 지금은 맞다. 건축주 입장에서 실제로 매일 만지는 건 플랫폼 앱이 아니라 폰 기본 앱과 음성이다. 그래서 "플랫폼이 Apple Home·Google Home으로 잘 내보내주는가"가 실사용 품질을 좌우한다. DALI→KNX→Matter처럼 여러 단계를 거치는 구성도 있지만 오늘은 생략.
