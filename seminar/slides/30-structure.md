...
<tr><td class="grp">그룹 · 페이드 · 동시성</td><td>방·존은 Zigbee 그룹 명령 하나로 <strong>동시</strong>. 페이드 시간 0.1초 단위 지정. 등 하나하나 따로 명령하면 팝콘</td><td>그룹·씬, 전환 효과 조절. 허브 밖 개별 명령 시 <strong>팝콘</strong></td><td>씬이 클라우드 실행, 팝콘 흔함</td><td>그룹·씬·브로드캐스트 명령은 버스 전체가 <strong>동시, 누락 없음</strong>. 주소별 개별 명령은 1200bps라 50개면 1~2초, 팝콘. 페이드는 표준 명령</td></tr>
