...
<li><strong>DALI</strong> — 드라이버마다 주소를 주고 디지털 버스로 명령하는 유선 조명 제어 표준. 미리 배선해야 한다</li>
...
- 기존 벽 스위치는 전원을 끊는 대신, 릴레이의 **입력 단자**에 물려 "토글하라"는 신호를 준다
- 그래서 벽 스위치를 봉인할 필요가 없다 — 벽에서도, 앱에서도 켜고 끌 수 있고 상태가 동기화된다
- 전용 스위치는 다른 길이다 — 제조사 전용선이나 전용 프로토콜로 연결된다

Note: 이너 릴레이는 스위치 박스가 아니라 전원이 있는 등기구 쪽 천장 매입박스에 들어간다. 기존 벽 스위치는 전원을 끊는 대신 릴레이로 들어가는 접점 입력이 된다 — 중성선 문제는 이렇게 우회한다. 흔한 DIY 실수: 스위치 뒤에 릴레이를 직렬로 달면 벽 스위치를 내리는 순간 릴레이가 죽어서 테이프로 봉인하게 된다. 입력 단자에 물리면 와이파이가 죽어도 벽 스위치는 로컬로 작동한다. 전용 스위치는 다른 길이다. 제조사 전용선이나 프로토콜로 붙는다. 기존 배선을 재활용하느냐, 전용 배선을 새로 까느냐의 갈림이다.
