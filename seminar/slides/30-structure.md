...
<img class="diagram" src="assets/diagrams/platform-stack.svg" style="max-height:380px">

- 스위치·센서·조명(드라이버)이 모두 연결되는 **허브**. 씬, 자동화, 센서 로직 — 집의 규칙이 여기서 돈다
- 조명만 보고 정하면 안 된다 — 냉난방·커튼·센서가 **나중에 같은 자리**에 붙는다
- 플랫폼이 갈리면(조명은 A, 냉난방은 B) 씬이 성립하지 않는다. 허브끼리 끌어오는 방법은 있지만 쉽지 않다 — **하나 정하고 전부 모은다**
