...
<div class="card"><h4 style="color:#ffb454;font-size:.8em;letter-spacing:.1em;margin-bottom:.5em">CC, 점광원용</h4>다운라이트·스폿. LED 전류를 직접 다루므로 <strong>미세한 디밍에 유리</strong>. 거리에 둔감. 등기구별 전류 규격(mA)을 맞춰야 하고, <strong>출력 전압 상한이 등기구 수를 정한다</strong></div>
...
Note: 드라이버 레이어 안의 두 부류. CC는 전류원이라 선이 길어져도 드라이버가 전압을 올려 전류를 지키고, CV는 전압원이라 선이 길면 끝에서 전압이 떨어진다. 이게 3막에서 "드라이버 위치"를 따로 얘기하는 이유다. CV 저가품은 PWM 주파수가 낮아 카메라에 줄이 가는 플리커가 흔하다. "여러 개를 한 드라이버에 묶어 비용을 줄일 수 없냐"는 질문이 자주 나온다. 원리상 직렬로 가능하지만 주거용 드라이버는 출력 상한이 33~45V, 정격이 10W 안팎이라 30V짜리 다운라이트는 하나가 끝이다. 여러 개를 묶으려면 고전압·고출력 상업용 드라이버가 필요하고 DC 120V(안전 특별저압)도 넘게 된다. 그래서 주거는 1:1이 표준이다. 질문이 나오면: 정전력(constant power) 타입이라는 세 번째 분류도 있지만 가로등 같은 고출력용이라 주거에선 거의 안 쓴다. 출력 전류를 딥스위치나 프로그래밍으로 고르는 CC 드라이버가 흔해서 등기구별 mA를 맞추기가 예전보다 쉽다. CV라고 정전류 제어가 없는 게 아니라 그 역할이 스트립 쪽으로 넘어간 것이다.
