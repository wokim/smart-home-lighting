...
- **씬.** 그룹 안에서 조명마다 밝기를 정해 저장한 것이다. 영화 모드가 이것이다
- **매핑.** 버튼과 센서에 씬을 걸어 두는 것이다. 이게 있어야 벽에서 손이 닿는다
- 방식이 달라도 하는 일은 같다. 차이는 **이 네 가지를 어디에 저장하고 누가 실행하느냐**다

Note: 제어 얘기의 척추다. DALI든 Hue든 Aqara든 이름만 다르고 네 가지는 똑같다. 뒤에 나오는 비교표도 기능 나열이 아니라 이 네 가지를 어떻게 처리하는지의 비교로 보면 된다. 주소는 자동으로 부여되기도 하고 손으로 정하기도 한다. 겹치는 묶음이 실무에서 유용하다. 거실 전체, 거실 간접등만, 1층 전체가 다 필요하고 같은 조명이 세 묶음에 들어간다. Hue는 이걸 방과 존으로 나눠 부르고, DALI는 그룹 16개로 처리한다. 이름만 다르고 하는 일은 같다. 그룹과 씬은 사람이 정해야 하고, 그게 설계 단계의 시나리오다.
